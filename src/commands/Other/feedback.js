  ...
  async execute(interaction) {
    ...
    if (subcommand === "bugreport") {
      ...
      if (cooldowns.has(userID)) {
        const cooldownEmbed = new EmbedBuilder().setTitle(
    ...
    if (subcommand === "suggestion") {
      ...
      if (cooldowns.has(userID)) {
        const cooldownEmbed = new EmbedBuilder().setTitle(
          `Please wait a few hours before sending a suggestion.`
        ).setColor("Red");
        const lastTime = cooldowns.get(userID);
        const cooldownTime = 60 * 60 * 1000; // 1 hour in milliseconds
        const timeLeft = (lastTime + cooldownTime) - Date.now();
    ...
    const embed2 = new EmbedBuilder()
      .setColor("Green")
      .setAuthor({ name: `We have recived your suggestion. Thanks for your time to make us better.` })
