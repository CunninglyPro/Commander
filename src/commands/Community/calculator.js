...
data: new SlashCommandBuilder()
    .setName('calculator')
    .setDescription('Summon a fully functional calculator within your discord.'),
async execute(interaction) {
    
    const idPrefix = 'calculator';
    const embed = new EmbedBuilder()
    .setColor("Yellow")
    .setDescription("```\nUse the buttons to navigate the calculator.\n```")
    ...
    collector.on('collect', async i => {
        ...
        if (value === "=") {
            try {
                ...
                data = "";
                extra = "An error occured, please press the 'AC' button to start over."
            }
        } else if (value === 'clear') {
            data = "";
            extra = "Use the button to navigate the calculator."
        ...
        i.update({ embeds: [new EmbedBuilder().setColor(`Yellow`).setDescription(`\`\`\`\n${data || extra}\n\`\`\``)], components: [row, row1, row2, row3, row4] })
